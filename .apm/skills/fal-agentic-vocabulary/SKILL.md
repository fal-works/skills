...
Under the Applicability principle, an antipattern name applies when the defined failure occurs, not merely when the case resembles a form mentioned in the definition. Each name marks what it takes as the problem, so one case can fall under several names.
