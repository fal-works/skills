...
The sections below counter these tendencies. Sections 1-8 follow the sequence of decisions made while writing:
...
- §4 In which words
- §5 In what structure
- §6 In what paragraphs and lists
- §7 In what sentences
- §8 How it fits its surroundings

Section 9 points to the Japanese-specific companion skill, and section 10 adds working discipline. A single flaw can break several sections at once; the overlap is intentional. Apply the sections while writing, and once more in a brief revision pass (§10).
...
- A document set split into overview and detail has already assigned homes. In such a set (a skill file and its references/ companion, a README and docs/, a spec and its appendices), case inventories, worked examples, and per-item explanations go to the detail layer, and the overview keeps its sections at uniform granularity. An overview section that outgrows its siblings (§8) usually holds detail that belongs in the detail layer; relocate it rather than trimming it.
...
## 6. One role per paragraph or list
...
## 8. Match the surrounding corpus
...
Japanese has failure modes of its own beyond what the sections above cover. The `fal-write-ja` skill owns these rules, and it applies to all Japanese output. Apply it together with this skill whenever the text is Japanese.
